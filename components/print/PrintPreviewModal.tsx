...
export default function PrintPreviewModal({
  ...
  const buildPrintHtml = () => {
    ...
    // For continuous form: width only, auto height
    // For sheet mode: width and height
    const pageRule = pageH 
      ? `@page { size: ${pageW}mm ${pageH}mm; margin: 10mm 12mm; }`
      : `@page { size: ${pageW}mm auto; margin: 5mm 5mm; }`;
    
    // For continuous form, add body styling to match the width
    const bodyStyle = paperMode === 'continuous'
      ? `body { width: ${pageW}mm; margin: 0 auto; background: white; }`
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ${bodyStyle}
    body { font-family: Arial, Helvetica, sans-serif; font-size: 10px; color: #111; padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    
    /* Continuous form specific styles */
    .continuous-mode { width: ${pageW}mm; min-height: auto; }
...
<body>${printRef.current.innerHTML}</body>
  ...
  const handlePrint = () => {
    ...
    // For continuous form, open with specific width to match 210mm
    // 210mm ≈ 794px at 96 DPI
    const windowWidth = paperMode === 'continuous' ? 850 : 900;
    const windowHeight = 700;
    
    const win = window.open('', '_blank', `width=${windowWidth},height=${windowHeight}`);
    if (!win) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(17,24,39,0.97)' }}>
      ...
      {paperMode === 'continuous' && (
        <div className="bg-amber-900 border-b border-amber-700 px-4 py-2 shrink-0">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 text-amber-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="text-xs text-amber-100 font-medium">Mode: Continuous Form 2 (Kertas Kontinu)</span>
            </div>
            <span className="text-xs text-amber-200">
              Lebar: {pageW}mm | Tinggi: Fleksibel (sesuai konten)
            </span>
          </div>
          <div className="mt-1.5 text-xs text-amber-200 flex items-start gap-2">
            <svg className="w-3.5 h-3.5 text-amber-300 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <strong className="text-amber-100">Tombol Print akan membuka dialog dengan instruksi lengkap.</strong> Anda akan diminta mengatur paper size di printer settings untuk continuous form (210mm lebar).
            </div>
          </div>
        </div>
      )}
          ...
          {useContentFrame ? (
            <div
              ...
              style={{
                ...
                height: paperMode === 'continuous' ? 'auto' : (pageHeightPx ? `${pageHeightPx}px` : 'auto'),
                margin: '0 auto',
