...
export default function PrintPreviewModal({ title, onClose, children }: PrintPreviewModalProps) {
  const printRef = useRef<HTMLDivElement>(null);
  const [zoom, setZoom] = useState(90);
  const [settings, setSettings] = useState<PrintSettings>({ paperSize: 'A4', orientation: 'portrait' });
  const [showSettings, setShowSettings] = useState(false);

  const dims = PAPER_DIMS[settings.paperSize];
  const pageW = settings.orientation === 'portrait' ? dims.w : dims.h;
  const pageH = settings.orientation === 'portrait' ? dims.h : dims.w;
...
<body>${printRef.current.innerHTML}</body>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(17,24,39,0.97)' }}>
      ...
      {/* Preview Area */}
      <div className="flex-1 overflow-auto flex justify-center py-6 px-4 bg-gray-700">
        <div
          style={{
            transform: `scale(${zoom / 100})`,
            transformOrigin: 'top center',
            width: `${pageW * (96 / 25.4) * (100 / zoom)}px`,
          }}
        >
          <div
            ref={printRef}
            style={{
              background: '#fff',
              width: `${pageW * (96 / 25.4)}px`,
              minHeight: `${pageH * (96 / 25.4)}px`,
              margin: '0 auto',
              padding: '38px 53px',
