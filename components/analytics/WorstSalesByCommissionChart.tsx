...
/**
 * Worst Sales By Commission Chart Component
 * 
 * Displays 10 sales persons with lowest commission using horizontal bar chart.
...
interface WorstSalesByCommissionChartProps {
  /** Optional company filter */
  companyFilter?: string;
}

export default function WorstSalesByCommissionChart({ companyFilter }: WorstSalesByCommissionChartProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      const params = new URLSearchParams({ type: 'worst_sales_by_commission' });
      if (companyFilter) {
      ...
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: Failed to fetch worst sales by commission data`);
      }
      ...
      if (result.success) {
        ...
        throw new Error('Failed to load worst sales by commission data');
      }
    } catch (err) {
      console.error('[WorstSalesByCommissionChart] Error fetching data:', err);
      ...
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-lg font-semibold text-gray-900">
            10 Sales Terendah Berdasarkan Komisi
          </h3>
        </div>
        <p className="text-sm text-gray-600">
          Sales dengan komisi terendah yang perlu perhatian
        </p>
      </div>
