/**
 * Integration Tests: Sales Invoice API
 * Task 3.6: Write integration tests untuk Sales Invoice API
 * 
 * Validates: Requirements 2.4, 2.5, 2.8, 15.3
 * 
 * Tests:
 * - Create invoice dengan discount_percentage
 * - Create invoice dengan discount_amount
 * - Create invoice dengan PPN 11%
 * - Create invoice dengan PPN + PPh 23
...
// Test 3: Create invoice with PPN 11%
async function testCreateInvoiceWithPPN(): Promise<TestResult> {
  let invoiceName: string | null = null;
  ...
  try {
    ...
    const hasTax = retrieved.taxes_and_charges === 'PPN 11%';
    ...
    if (hasTax && taxAmountCorrect && grandTotalCorrect) {
      return {
        name: 'Create Invoice with PPN 11%',
        passed: true,
        invoiceName: invoiceName || undefined
      };
    } else {
...
// Test 4: Create invoice with discount + PPN
async function testCreateInvoiceWithDiscountAndPPN(): Promise<TestResult> {
  ...
  try {
    ...
    if (discountCorrect && netTotalCorrect && taxAmountCorrect && grandTotalCorrect) {
      return {
        name: 'Create Invoice with Discount + PPN',
        passed: true,
        invoiceName: invoiceName || undefined
      };
    } else {
...
async function runIntegrationTests() {
  ...
  console.log('🧪 Integration Tests: Sales Invoice API');
  console.log('   Task 3.6: API integration tests');
  console.log('   Validates: Requirements 2.4, 2.5, 2.8, 15.3');
  console.log('='.repeat(60));
  ...
  for (const invoiceName of createdInvoices) {
    try {
      ...
      console.warn(`   Warning: Could not delete ${invoiceName}`);
    }
  }
