  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Gagal memuat penyesuaian stok');
      }
    } catch (err) {
      console.error('Error fetching stock adjustment:', err);
      setError('Gagal memuat penyesuaian stok');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Penyesuaian Persediaan & Stock Opname</h1>
          <p className="text-sm text-gray-500">Jurnal penyesuaian stok per periode (selisih qty & nilai)</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <p className="text-sm text-green-600 font-medium">Total Nilai</p>
          <p className="text-2xl font-bold text-green-900">Rp {totalAmount.toLocaleString('id-ID')}</p>
        </div>
      </div>
