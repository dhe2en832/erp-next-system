...
import { PurchaseInvoiceWithItems } from '@/types/purchase-invoice-details';
...
export default function PurchaseInvoiceDetailsPage() {
  ...
  // State management
  const [data, setData] = useState<PurchaseInvoiceWithItems[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/finance/reports/purchase-invoice-details?${params}`, {
        credentials: 'include'
      });
      ...
      if (result.success) {
        ...
        if (typeof window !== 'undefined') {
          (window as any).__purchaseInvoiceData = result.data || [];
        }
      } else {
  ...
  const filteredData = useMemo(() => {
    return data.filter(invoice => {
      const matchSupplier = !filterSupplier || 
        ...
        invoice.supplier_name?.toLowerCase().includes(filterSupplier.toLowerCase()) ||
        invoice.name.toLowerCase().includes(filterSupplier.toLowerCase());
      
      const matchStatus = !filterStatus || invoice.status === filterStatus;
      
      return matchSupplier && matchStatus;
    });
  }, [data, filterSupplier, filterStatus]);
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">
          Laporan Detail Pembelian Per Faktur
        </h1>
        <button
      ...
      <FilterSection
        ...
        searchValue={filterSupplier}
        onSearchChange={setFilterSupplier}
        searchPlaceholder="Cari pemasok atau nomor faktur..."
        additionalFilters={
          <div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-900 mt-1">{invoice.supplier_name || invoice.supplier}</p>
      ...
      {showPrint && (
        <PrintPreviewModal
          title={`Laporan Detail Pembelian Per Faktur — ${selectedCompany}`}
        ...
        >
          <div className="p-8 bg-white">
            <div className="text-center mb-6">
              ...
              <h3 className="text-lg font-semibold mt-2">Laporan Detail Pembelian Per Faktur</h3>
                ...
                {filteredData.map((invoice) => (
                  <React.Fragment key={invoice.name}>
                    <tr className="border-b border-gray-200">
                      ...
                      <td className="py-2 px-2">{invoice.supplier_name || invoice.supplier}</td>
