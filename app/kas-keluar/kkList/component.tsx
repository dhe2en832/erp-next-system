...
export default function KasKeluarList() {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      // Tambahkan filter khusus untuk kas keluar
      params.append('kas_type', 'keluar');
      ...
      if (result.success) {
        ...
        setEntries(data);
        setError(data.length === 0 ? `Tidak ada kas keluar untuk perusahaan: ${company}` : '');
      } else {
        setError(result.message || 'Gagal memuat kas keluar');
      }
    } catch {
      setError('Gagal memuat kas keluar');
    } finally {
      setLoading(false);
    }
  ...
  if (loading && entries.length === 0) {
    return <LoadingSpinner message="Memuat Kas Keluar..." />;
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-4 sm:px-6 lg:px-8 sticky top-0 z-30">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button
            onClick={() => router.push('/kas-keluar/kkMain')}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="px-4 py-4">
                  {isMobile ? (
                    ...
                    <div className="space-y-3">
                      <div 
                        onClick={() => router.push(`/kas-keluar/kkMain?name=${encodeURIComponent(entry.name)}`)}
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-gray-100 mt-2">
                          <div>
                            <p className="text-xs text-gray-500 mb-0.5">Total Pengeluaran</p>
                    ...
                    <div className="grid grid-cols-12 gap-4 items-center">
                      ...
                      >
                        <p className="text-sm font-semibold text-indigo-600 truncate">{entry.name}</p>
                        <p className="text-xs text-gray-500 mt-0.5">Kas Keluar</p>
                      </div>
                      <div 
                      ...
                      <div 
                        ...
                        onClick={() => router.push(`/kas-keluar/kkMain?name=${encodeURIComponent(entry.name)}`)}
                      >
                        <p className="text-sm font-semibold text-rose-600">{formatCurrency(entry.total_credit)}</p>
                      </div>
                      <div 
                        className="col-span-2 cursor-pointer"
                        onClick={() => router.push(`/kas-keluar/kkMain?name=${encodeURIComponent(entry.name)}`)}
          ...
          {entries.length === 0 && !loading && (
            <div className="text-center py-16 px-4">
              ...
              <p className="mt-4 text-sm text-gray-500">Tidak ada kas keluar yang ditemukan</p>
              <button
                onClick={() => router.push('/kas-keluar/kkMain')}
