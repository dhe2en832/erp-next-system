...
export async function POST(
  ...
  try {
    ...
    // Call ERPNext receive method
    const response = await fetch(`${ERPNEXT_API_URL}/api/resource/Purchase Order/${id}/receive`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      return NextResponse.json(
        { success: false, message: data.exc || data.message || 'Failed to receive purchase order' },
