      ...
      if (data.exc) {
        ...
        try {
          ...
          if (excData.exc_type === 'MandatoryError') {
            errorMessage = `Missing required field: ${excData.message || 'Unknown field'}`;
          } else if (excData.exc_type === 'ValidationError') {
            errorMessage = `Validation error: ${excData.message || 'Unknown validation error'}`;
          } else if (excData.exc_type === 'LinkValidationError') {
            errorMessage = `Invalid reference: ${excData.message || 'Unknown reference error'}`;
          } else if (excData.exc_type === 'PermissionError') {
            errorMessage = `Permission denied: ${excData.message || 'Unknown permission error'}`;
          } else {
            errorMessage = excData.exc_type && excData.message 
              ? `${excData.exc_type}: ${excData.message}`
              : excData.exc_type || excData.message || 'Failed to create delivery note';
          }
        } catch (_e) {
        ...
        errorMessage = data.exc_type;
      } else if (data.exc_message) {
        errorMessage = data.exc_message;
      } else {
