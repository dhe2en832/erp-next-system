...
export async function GET() {
  try {
    console.log('=== CHECK SPECIFIC DN MAT-DN-2026-00001 ===');
    
    const authString = Buffer.from(`${process.env.ERP_API_KEY}:${process.env.ERP_API_SECRET}`).toString('base64');
    
    // Get specific DN MAT-DN-2026-00001
    console.log('Getting specific DN MAT-DN-2026-00001...');
    const dnResponse = await fetch(`${ERPNEXT_API_URL}/api/resource/Delivery Note?fields=["*"]&filters=[["name","=","MAT-DN-2026-00001"]]`, {
      method: 'GET',
      headers: {
    ...
    if (dnData.data && dnData.data.length > 0) {
      ...
      return NextResponse.json({
        ...
        validation: {
          is_valid_for_filtering: soReferences.size > 0,
          can_test_filtering: soReferences.size > 0,
          recommended_test_case: soReferences.size > 0 ? {
            so_to_test: Array.from(soReferences)[0],
            expected_behavior: 'SO should be HIDDEN from dialog',
            test_description: `SO ${Array.from(soReferences)[0]} should NOT appear in Sales Order Dialog because DN ${dn.name} exists`
          } : null
        }
      });
    } else {
